...
const LinePortrait: FC = () => {
  ...
  const handleRestartAnimation = () => {
    if (svgLinePathRef.current) {
      setIsRestartButtonShown(false)
      svgLinePathRef.current.classList.remove(DRAW_ANIMATION_CLASSNAME)
      svgLinePathRef.current.classList.add(ERASE_ANIMATION_CLASSNAME)
      timeoutRef.current = setTimeout(() => {
        svgLinePathRef.current?.classList.remove(ERASE_ANIMATION_CLASSNAME)
        svgLinePathRef.current?.classList.add(DRAW_ANIMATION_CLASSNAME)
        timeoutRef.current = setTimeout(() => {
          setIsRestartButtonShown(true)
        }, 6000)
      }, 600)
  ...
  return (
    <div className="relative">
      <svg
        ...
        className="h-80 md:h-96 lg:h-[512px]"
      >
        <path
          ref={svgLinePathRef}
          className={DRAW_ANIMATION_CLASSNAME}
      ...
      <ButtonIcon
        disabled={!isRestartButtonShown}
        className="absolute -right-10"
        aria-hidden="true"
        onClick={handleRestartAnimation}
      >
        <RefreshIcon
          className={cn("h-6 w-6 transition-opacity md:h-7 md:w-7", {
            "opacity-0": !isRestartButtonShown,
          })}
        />
      </ButtonIcon>
    </div>
